...
```
Untrack means the file will be deleted from the remote repository on next commit, but will remain locally.
...
- [git restore \<filename1> \<filename2> ...](https://git-scm.com/docs/git-restore) - To discard unstaged (**before `git add`**) and uncommitted (**before `git commit`**) file changes in the local working directory and restore the originals from Head.
